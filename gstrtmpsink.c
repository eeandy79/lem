...
gst_rtmp_sink_class_init (GstRTMPSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_MUTE,
      g_param_spec_string ("mute", "RTMP mute", "RTMP output",
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MUTE:
	  {
		  const gchar *muteStr = g_value_get_string(value);
		  pthread_mutex_lock(&sink->lock);
		  if (strcmp((const char*)muteStr, "false") == 0) {
			  sink->mute = FALSE;
		  } else {
			  sink->mute = TRUE;
		  }
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_LOCATION:
      ...
      break;
	case PROP_MUTE:
	  g_value_set_string (value, sink->mute == TRUE ? "true" : "false");
	  break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
