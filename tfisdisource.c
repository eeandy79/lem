...
static void tfi_sdi_src_class_init (TfiSdiSrcClass * klass)
{
...
static void tfi_sdi_src_init (TfiSdiSrc *src)
{
	src->pad_counter = 0;
	src->running = FALSE;
...
static void video_frame_free (void *data)
{
...
static void audio_packet_free (void *data)
{
...
{
public:
	void PushBuffer(GstPad *pad, GstBuffer *buf);

	bool RegisterContext(GstPad *pad);

	void Start() {
		pthread_create(&m_thread, NULL, &Pacer::worker, this);
	}
	...
	virtual HRESULT STDMETHODCALLTYPE
				...
				IDeckLinkAudioInputPacket * audio_packet)
		{
			...
			if (good_video_frame) {
				...
				video_frame->GetBytes(&rawbuf);
				VideoFrame *vf = (VideoFrame *) g_malloc0 (sizeof (VideoFrame));

				GstBuffer *buffer = gst_buffer_new_wrapped_full ((GstMemoryFlags) GST_MEMORY_FLAG_READONLY,
						(gpointer) rawbuf, bufsize, 0, bufsize, vf,
			...
			if (audio_packet && m_bCanStart)
			{
				...
				void* rawbuf;
				audio_packet->GetBytes(&rawbuf);
				AudioPacket *ap = (AudioPacket *) g_malloc0 (sizeof (AudioPacket));

				video_frame->GetStreamTime (&stream_time, &stream_duration, GST_SECOND);

				GstBuffer *buffer = gst_buffer_new_wrapped_full ((GstMemoryFlags) GST_MEMORY_FLAG_READONLY,
						(gpointer) rawbuf, bufsize, 0, bufsize, ap,
